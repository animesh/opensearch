process AA_STAT {
    ...
    input:
    tuple val(sample_id), val(mzml_file), path(pepxml_file)

    output:
    tuple val(sample_id), path("${sample_id}.AA_statm"), emit: aa_stat_dir
    ...
    ${params.aa_stat_bin} \
        ...
        --mzml ${mzml_file} \
        --pepxml ${pepxml_file} \
        --dir ${sample_id}.AA_statm

    # Extract number of mass shifts and recommended variable mods for general stats
    n_shifts=\$(grep -c '^[+-]' ${sample_id}.AA_statm/aa_statistics_table.csv 2>/dev/null || echo 0)
    n_loc=\$(tail -n +2 ${sample_id}.AA_statm/localization_statistics.csv 2>/dev/null | wc -l || echo 0)
